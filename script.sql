drop database dbDistribuiaaraEduGui;

-- criando db e pondo em uso
create database dbDistribuiaaraEduGui;
use dbDistribuiaaraEduGui;
...
delimiter $$
create procedure spExibirProduto()
begin
	select * from tbProduto;
end $$

call spExibirProduto;
...
-- EXERCÍCIO 37
select * from tbcliente inner join tbendereco  on tbcliente.cep = tbendereco.cep inner join tbBairro on tbendereco.IdBairro = tbBairro.IdBairro Inner join tbCidade  on tbendereco.IdCidade = tbcidade.IdCidade  inner join tbuf  on tbendereco.IdUF = tbUF.IdUF;
